...
import { FacultyRoomSheet } from './FacultyRoomSheet';
  ...
  return (
    ...
    >
      <div className="space-y-0 pb-2">
        ...
        {/* ── TABS ─────────────────────────────────────────────────── */}
        {building.facultyRooms && building.facultyRooms.length > 0 && (
          <div className="px-4 pt-3 pb-0 flex gap-2 border-b border-slate-100">
            <button
              type="button"
              onClick={() => setActiveTab('overview')}
              className={`pb-2.5 px-1 text-xs font-extrabold border-b-2 transition-colors ${
                activeTab === 'overview'
            ...
            <button
              ...
              className={`pb-2.5 px-1 text-xs font-extrabold border-b-2 transition-colors flex items-center gap-1 ${
                activeTab === 'faculty'
                  ? 'border-cyan-500 text-cyan-600'
                  : 'border-transparent text-slate-400 hover:text-slate-600'
              }`}
        ...
        {/* ── FACULTY ROOMS TAB ─────────────────────────────────────── */}
        {activeTab === 'faculty' && building.facultyRooms && (
          <div className="pt-2">
            <FacultyRoomSheet
              buildingName={building.name}
              facultyRooms={building.facultyRooms}
              classrooms={building.popularRooms}
            />
          </div>
        )}
